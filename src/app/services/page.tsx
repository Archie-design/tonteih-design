...
export default function ServicesPage() {
  return (
    <main id="main" className="pt-24">
      ...
      {/* Services Bento Grid */}
      <section className="px-8 py-12 max-w-7xl mx-auto" aria-labelledby="services-heading">
      ...
      <section className="bg-[var(--color-surface-container-low)] py-32 mt-20">
        <div className="max-w-7xl mx-auto px-8">
          ...
          <div className="relative space-y-4 md:space-y-0">
            {[
              { num: '01', title: '初步諮詢與溝通', desc: '我們在工作室或您的住所進行訪談，深入了解您的生活型態、審美偏好與功能需求。每一個細節都至關重要。', align: 'lg:mr-auto', bg: 'bg-[var(--color-surface-container-lowest)]', mt: '' },
              { num: '02', title: '全方位現場勘測', desc: '我們的技術團隊會進行精密的空間測繪，在設計開始前找出結構潛力與限制因素。', align: 'lg:ml-auto', bg: 'bg-[var(--color-surface-container-high)]', mt: 'lg:-mt-12' },
              { num: '03', title: '方案設計與固定報價', desc: '我們提供 3D 視覺模擬圖，並附上詳盡的細項預算表。我們承諾的報價即是您的最終支付價格，絕不巧立名目。', align: 'lg:mr-auto', bg: 'bg-[var(--color-surface-container-lowest)]', mt: 'lg:-mt-12' },
              { num: '04', title: '職人精神施工執行', desc: '內部專案經理全程監製，統籌各項工程職人。您可以透過專屬客戶端，每週收到施工進度照片更新。', align: 'lg:ml-auto', bg: 'bg-[var(--color-primary)]', mt: 'lg:-mt-12', dark: true },
              { num: '05', title: '驗收交屋與保固', desc: '我們將與您逐項確認完工細節。所有工程均享有全方位的 5 年結構保固承諾。', align: 'lg:mr-auto', bg: 'bg-[var(--color-surface-container-lowest)]', mt: 'lg:-mt-12' },
              ...
              <div key={num} className={`relative ${bg} p-10 md:p-16 rounded-xl border border-[var(--color-outline-variant)]/10 max-w-4xl ${align} ${mt} group`} style={{ zIndex: (i + 1) * 10 }}>
                <div className="flex flex-col md:flex-row gap-8 items-start">
                  ...
                  <div>
                    ...
                    <p className={`font-body leading-relaxed max-w-lg ${dark ? 'text-[var(--color-on-primary-container)]' : 'text-[var(--color-on-surface-variant)]'}`}>{desc}</p>
                  </div>
                </div>
              </div>
            ))}
